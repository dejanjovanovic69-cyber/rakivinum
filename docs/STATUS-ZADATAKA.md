...
**Poslednji zapis:** 2026-05-03 — **Worker skaliranje (javni GET):** `servePublicCached` sada koristi **Cloudflare `caches.default`** (deljeni edge odgovor po normalizovanom URL-u) + **in-flight koalescenciju** u izolatu (stotine paralelnih zahteva na isti URL → jedan Firestore handler po izolatu dok traje fetch). I dalje važi: **različiti URL-ovi** (npr. `club-memberships/{visitorId}`, `products-by-ids?ids=…`) i dalje prave posebne read-ove; za „ceo katalog za sve“ sledeći korak je materijalizacija (cron → R2/KV) ako treba još niže.
