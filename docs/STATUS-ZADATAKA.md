...
**Worker (CORS + deploy):** odgovori izlažu **`x-cache-status`** preko `Access-Control-Expose-Headers` (edge meter u konzoli vidi `cache:` posle reload-a). **Deploy:** `rakivinum-api` verzija **`3c589512-7b15-494e-995d-a0e5ee5c4e3a`** (2026-05-04). U DevTools → Network i dalje uvek vidiš header na odgovoru API-ja.

**Frontend (Firestore / dupli poziv):** `Home.tsx` — `home-bundle` više **ne** duplira Worker poziv pri povratku sa etikete jer `shouldRunRefresh` sada koristi **isti** ključ za mount i za `focus`/`visibility` (ranije `home:initial-bundle` vs `home:focus-bundle`). **Deploy (2026-05-04):** Cloudflare Pages `master.rakivinum.pages.dev` + Firebase hosting `gen-lang-client-0889534325.web.app` (isti `dist`); ako `rakivinum.com` ide preko CF, trebalo bi da povuče novi bundle posle propagacije.
