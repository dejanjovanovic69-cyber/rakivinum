...
**Poslednji zapis:** 2026-05-03 — **Home → jedan Worker odgovor:** novi **`GET /api/public/home-bundle?visitor=`** (jedan keširani poziv) vraća članstva + akcije + dnevnu preporuku + `distilleryNames`; `Home.tsx` više ne pali 4 odvojena edge poziva + `distilleries-by-ids`. U handleru: manji list `products` (**16**) za dnevni izbor; isto za **`/api/public/daily-recommendations`**. **Worker skaliranje (ranije):** `caches.default` + in-flight koalescencija u `servePublicCached`.
