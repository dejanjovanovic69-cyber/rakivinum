...
**Poslednji zapis:** 2026-05-03 — **Home bundle (cap):** `home-bundle` handler sada drži **~≤40** Firestore dok./odgovor (članstva **12**, akcije list **14**→**12** UI, `products` **8** za dnevni izbor, imena destilerija **max 6**); **`/api/public/daily-recommendations`** isto **8** proizvoda. **Home.tsx:** `useRef` da se refresh ne preklapa. **Ranije istog dana:** jedan **`/api/public/home-bundle`**, `caches.default` + in-flight u Workeru.
