...
**Poslednji zapis:** 2026-05-05 — **Admin panel — ceo dan jedan „hladan“ prolaz:** glavni uzrok ponovljenih pikova bio je **`writeCache(..., ADMIN_PANEL_10M)`** — `readCache` je isticao posle **10 min**, pa je svaki povratak u tab ponovo palio Firestore. Uveden **`REFRESH_INTERVAL.ADMIN_PANEL_24H`** (24h TTL u `localStorage` + isti interval za `shouldSkipAdminNetworkAfterCache`); bump admin keš ključeva (`*_v2` / proizvodi `v3`) da se odbaci stari kratki TTL; **`fetchRecentRatings`** ispravljen da koristi isti `shouldSkipAdminNetworkAfterCache` kao ostali fetch-evi (ranije je ostala stara grana). **`AdminAudit.tsx`:** isti 24h TTL + `*_v2` keš ključevi. **Online brojač u Adminu:** gate sa **1h** (`USER_LIGHT_1H`). I dalje važi raniji fix: `refreshGate` bez `last ?? 0` + `seedRefreshGate` pri prvom kešu u sesiji.  
