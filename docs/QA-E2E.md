...
Trenutno smoke pokriva: početnu, Community (default + tab deep linkovi + `?tab=search&q=…` / `pf=…` + `?tab=compare&lq=…&rq=…` / `cf=…` iz URL-a), `/distilleries`, `/collection` (gost prazan ili „Arhiva“), `/menu` (Gost/Korisnik), `/scan`, `/radionica`, `/my-clubs`, `/activate` (bez tokena — forma), `/label/…` i `/distillery/…` (nepostojeći id — poruka o nedostupnosti).
